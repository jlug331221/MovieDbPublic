...
},{"typeahead.js":19,"typeahead.js/dist/bloodhound.js":18}],18:[function(require,module,exports){
/*!
 * typeahead.js 0.11.1
...
$(function () {
    ...
    $('#search').suggest({
        identifier: 'blah',
        default: [{ 'title': 'Gravity',
            'thumb': 'http://i.imgur.com/fbm4rs0.png' }, { 'title': 'Frozen',
            'thumb': 'http://i.imgur.com/lmase6g.jpg' }, { 'title': 'Taxi Driver',
...
},{"./suggest.js":21,"bootstrap":2,"bootstrap-datepicker":1,"jquery":15,"moment":16,"typeahead.js-browserify":17}],21:[function(require,module,exports){
'use strict';
...
(function ($) {
    ...
    $.fn.suggest = function (options) {
        ...
        var settings = $.extend({
             ...
             */
            template: function template(suggestion) {
                for (var first in suggestion) {
        ...
        if (settings.prefetchUrl) remoteFetch(settings.prefetchUrl, false);
        ...
        frame.input.on('focusin', function () {
            if (query) frame.wrapper.addClass('open');
        });
        ...
        frame.input.on('focusout', function () {
            ...
            if ($(frame.contentWrapper.selector + ':hover').length === 0) frame.wrapper.removeClass('open');
        });
        ...
        frame.contentWrapper.on('mouseleave', function () {
            if (!frame.input.is(':focus')) frame.wrapper.removeClass('open');
        });
        ...
        function remoteFetch(url, repop) {
            if (remoteReady) {
                ...
                ajax({
                    ...
                    if (repop) repopSuggestions();
                }, function (xhr) {
                    console.error('Suggest Remote Error: GET ' + url + ' ' + xhr.status + ' ' + xhr.statusText);
     ...
     */
    var filter = function filter(data, key, regexp) {
        var indexes = [];
        $.map(data, function (item) {
            if (regexp.test(item[key])) indexes.push(data.indexOf(item));
        });
        return indexes;
    };
     ...
     */
    var tokenSearchRegExp = function tokenSearchRegExp(tokens) {
    ...
    var tokenize = function tokenize(str) {
        return $.map(str.match(/"(.*?)"|[^ ]+/g), function (term) {
            term = term.replace(/(["])/g, '');
            return term;
        });
    };
     ...
     */
    var replaceWildcard = function replaceWildcard(string, wildcard, replacement) {
     ...
     */
    var render = function render(input, id) {
